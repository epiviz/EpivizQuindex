...
MAX_DEPTH = 20
item_size = 72
leaf_size = 40 + 1 + ((MAX_ITEMS + 1) * itemSize)
parent_size = 40 + 1 + 32
...
class _QuadTree(object):
    ...
    def _intersect(self, rect, f_path, offset = None, results=None, uniq=None):
        if results is None:
        ...
        with open(f_path, 'rb') as f:
            f.seek(offset)
            (x, y, width, height, depth, isLeaf) = unpack("ddddl?", self.f.read(41))
            ...
            if not isLeaf:
                ...
                if self.box_intersect(rect, (x - width/2, y, x, y + height/2)):
                    self._intersect(rect, f_path,children[1], results, uniq)
                if self.box_intersect(rect, (x - width/2, y - height/2, x, y)):
                    self._intersect(rect, f_path, children[0], results, uniq)
                if self.box_intersect(rect, (x, y - height/2, x + width/2, y)):
                    self._intersect(rect, f_path, children[2], results, uniq)
                if self.box_intersect(rect, (x, y, x + width/2, y + height/2)):
                    self._intersect(rect, f_path, children[3], results, uniq)
    ...
    def convert_to_disk(self, position):
        ...
        if self.children:
            ...
            barray += (MAX_ITEMS - len(nodes)) * pack("llllldddd", 0,0,0,0,0,0,0,0,0)
